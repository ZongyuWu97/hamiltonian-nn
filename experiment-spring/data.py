...
import autograd      # <-- still imported for consistency, though not used
...
##########################################################
# Unchanged: defines the true Hamiltonian H(q,p) = q^2 + p^2
##########################################################
...
def get_trajectory(t_span=[0,3], timescale=100, radius=None, y0=None, noise_std=0.1, **kwargs):
    ...
    y0 = y0 / np.sqrt((y0**2).sum()) * radius  # place on desired radius

    # 2) Solve the "true" Hamiltonian system to get (q_true, p_true)
    spring_ivp = solve_ivp(fun=dynamics_fn, t_span=t_span, y0=y0,
